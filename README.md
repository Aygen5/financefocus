...
FinanceFocus; bireysel gelir-gider dengesini kontrol altında tutmak, stratejik bütçe disiplini sağlamak, küresel varlık portföylerini anlık izlemek ve finansal geleceği yapay zeka destekli tahmin motorları ile simüle etmek amacıyla geliştirilmiş uçtan uca kurumsal bir finansal yönetim platformudur.
...
- **Finansal Özgürlük ve Hedef Planlayanlar**: Birikim hedeflerini (Ev, Araba, Acil Durum Fonu vb.) takip edip gelecek projeksiyonlarını simüle etmek isteyenler.
...
2. **FinanceFocus.Application**: İş mantığı servislerini (`FinancialEngineService`, `BudgetService`, `GoalService` vb.), DTO eşlemelerini (`AutoMapper`), doğrulama kurallarını (`FluentValidation`) ve servis arayüzlerini kapsar.
3. **FinanceFocus.Infrastructure**: PostgreSQL veritabanı erişimi (`FinanceFocusDbContext`), EF Core konfigürasyonları, JWT Token Üreteci, MemoryCache ve Hangfire arka plan işlerini barındırır.
4. **FinanceFocus.API**: HTTP isteklerini karşılayan Controller sınıfları, Global Exception Handler middleware, Serilog loglama ve Swagger dokümantasyon katmanı.
5. **Frontend Feature-Based Architecture**: React tarafı modüler (feature-based) yapıda kurgulanmıştır. Her modül (`activity`, `auth`, `budget`, `dashboard`, `financialHealth`, `forecast`, `goals`, `notifications`, `portfolio`, `reports`, `settings`, `subscriptions`, `transactions`) kendi bileşenlerini, slice'larını ve servislerini barındırır.
...
### 3. Bütçe Planlayıcı (Budget Planner)
...
### 9. Bildirim Merkezi (Notifications)
...
### 10. Aktivite Günlüğü (Activity Log)
...
Her DTO için özel validator sınıfları yazılmıştır. Örneğin `CreateTransactionValidator` veya `CreateBudgetValidator` ile tutarın sıfırdan büyük olması, kategorinin boş olmaması ve tarihlerin geçerliliği sunucu seviyesinde kontrol edilir.
...
### Neler Test Ediliyor?
- **Backend**: Servislerin doğru DTO döndürmesi, hesaplama motoru çıktıları, yetkisiz erişim durumları, CRUD operasyonları.
...
- [ ] **Yapay Zeka Destekli Harcama Danışmanı**: Ollama / OpenAI entegrasyonu ile kişiselleştirilmiş bütçe tavsiyeleri.
