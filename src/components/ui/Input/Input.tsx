...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  (
    { className = "", label, error, helperText, icon, leftIcon, rightIcon, id, disabled, ...props },
    ref,
  ) => {
    ...
    return (
      <div className="w-full text-left space-y-1.5">
        ...
        <div className="relative">
          ...
          {rightIcon && (
            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 z-10">
              {rightIcon}
            </span>
          )}
        </div>
