...
const Checkbox = forwardRef<HTMLInputElement, CheckboxProps>(
  ({ label, error, className = "", containerClassName = "", disabled = false, ...props }, ref) => {
    ...
    const checkboxStyles = `
      h-4 w-4 rounded border-slate-300 dark:border-slate-800 text-brand-500 bg-white dark:bg-slate-900 focus:ring-brand-500/20 focus:ring-2 focus:ring-offset-0 transition-all duration-150 cursor-pointer disabled:pointer-events-none disabled:opacity-50
